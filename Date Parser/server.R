# The to parse datetime is to convert the datetime into ISO format. DateTime column comes as a character
# And then parse it to ISO datetime format.
...
shinyServer(function(input,output,session){
  ...
  getDataFrame <- reactive({
    startTime <<- Sys.time()
    validate(
      need(input$file != ""||input$text != "", "Please select a data set")
    )
    ...
    if(endsWith(file_in$name,".xlsx")|| endsWith(file_in$name,".xls")){
      datf <- read.xlsx(file_in$datapath,1)
      if(!is.na(convertToDate(datf[,1][1]))){
        datf[,1] <- convertToDate(datf[,1])
      }
      return(datf)
    }
    else{
      datf <- read_csv(x,col_types =  cols(.default = col_character()), show_col_types = TRUE, trim_ws = TRUE, skip = 0, guess_max = 20000, lazy = should_read_lazy(),
                       name_repair = "unique", skip_empty_rows = TRUE, locale = default_locale())
  ...
  output$table1 <- renderTable({
    ...
    datf <- na.omit(datf)
    print(datf)
    sampleDateTime <- head(datf[[1]], 10)
    dateTimeFormat <- findDateTimeFormat(sampleDateTime)
    print(dateTimeFormat)
    dateTimeISO <- convertToISOFormat(dateTimeFormat, datf[[1]])
    as.character(dateTimeISO)
  })
})